...
def get_course_summary(course_id: str = COURSE_ID) -> CourseSummary:
    ...
    if course_id == NONGBO_COURSE_ID:
        return CourseSummary(
            id=NONGBO_COURSE_ID,
            title="农博后台管理系统项目实训",
            subtitle="按《Web应用系统开发一》课程标准组织路线，落到真实农博后台需求、数据库、接口和 SpringBoot/Vue 源码",
            progress=0,
    ...
    modules = [
        {
            ...
            "title": "项目1：需求认知与分层架构",
            "summary": "按课程标准“Spring简介与Web结构”要求，从农博需求、接口和源码目录理解 B/S、三层架构与请求链路。",
            "standard_unit": "项目1 Spring框架核心 - 1.1 Spring简介与Web结构",
            "objectives": ["描述农博后台的 B/S 架构和三层职责", "串起 Vue、Controller、Service、数据库的一次请求链路", "定位真实源码中的 Controller、Service、Entity/Mapper 和配置文件"],
            ...
            "practice_title": "梳理农博后台的一次完整请求链路",
            "practice_description": "阅读课程标准项目1要求和农博真实需求，补全从前端页面到后端接口、业务服务、数据库表的调用说明。",
            "checklist": ["说明 B/S 架构、前端 Vue 和后端 Spring Boot 的职责边界", "包含 Controller -> Service -> Entity/Mapper -> 数据库链路", "引用农博需求或接口规范中的真实模块名称", "指出统一返回 Result<T> 或接口前缀等项目规范"],
        },
        {
            "id": "nongbo-ioc-di-layering",
            "title": "项目1：IoC/DI 与三层解耦",
            "summary": "按课程标准 IoC/DI、Bean 管理要求，借农博真实 Controller 注入 Service 的写法理解解耦。",
        ...
        },
        {
            ...
            "summary": "对照课程标准的 AOP 与事务管理要求，分析农博业务中日志、事务和数据一致性应该落在哪里。",
        ...
        },
        {
            ...
            "summary": "按课程标准 Spring Boot 核心、自动配置和 MyBatis-Plus 整合要求，理解农博后台快速开发结构。",
            ...
            "practice_title": "说明农博项目的 Spring Boot 自动配置落点",
            "practice_description": "围绕启动类、application.yml、Mapper/Service 结构，补全项目如何通过 Spring Boot 和 MyBatis-Plus 简化开发。",
            "checklist": ["提到启动类、配置文件或自动配置机制", "说明数据库连接、上传路径或跨域配置中的至少一类配置", "说明 MyBatis-Plus/Mapper 如何简化 CRUD", "结合农博真实源码路径，不泛泛谈概念"],
        },
        {
        ...
        },
        {
            ...
            "docs": [COURSE_STANDARD_DOC, PROJECT_DOC_DIR / "01-项目需求分析.md", ROOT_NONGBO_SQL],
            "practice_title": "准备农博后台成果展示的数据依据",
        ...
        },
        {
            ...
            "summary": "按课程标准过程驱动与综合项目展示要求，完成农博 Vue 请求封装、Pinia、路由和真实接口联调复盘。",
