  ...
  <div class="video-course">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="header-title">视频课程管理</span>
      </div>
    </div>
      ...
      <el-form :model="queryParams" :inline="true" class="search-form">
        ...
        <el-form-item label="课程分类">
          <el-select v-model="queryParams.category" placeholder="请选择课程分类" clearable>
            ...
            <el-option label="设施农业" value="设施农业" />
            <el-option label="养殖技术" value="养殖技术" />
            <el-option label="经营管理" value="经营管理" />
            <el-option label="农机技术" value="农机技术" />
          </el-select>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="课程时长" prop="duration">
          <el-input v-model="dialogForm.duration" placeholder="例如：45分钟" />
        </el-form-item>
        <el-form-item label="视频链接" prop="videoUrl">
          <div class="file-input-row">
            <el-input v-model="dialogForm.videoUrl" placeholder="请输入视频URL或上传视频文件" clearable />
            <el-upload
            ...
            >
              <el-button :icon="VideoCamera">上传视频</el-button>
            </el-upload>
          </div>
        </el-form-item>
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="视频链接" :span="2">
          <el-link :href="viewData.videoUrl" target="_blank" type="primary">
            {{ viewData.videoUrl }}
          </el-link>
        </el-descriptions-item>
...
import request, { exportCsv, uploadFile, uploadImage } from '../../utils/request'
...
// 分类列表
const categories = ['种植技术', '设施农业', '养殖技术', '经营管理', '农机技术']
...
const getList = async () => {
  loading.value = true
  try {
    const res: any = await request.get('/dev-api/yjnb/videoCourse/list', {
      params: queryParams
    })
    if (res.code === 200) {
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      try {
        const url = dialogForm.id ? '/dev-api/yjnb/videoCourse' : '/dev-api/yjnb/videoCourse'
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(`确定要删除课程${row.title}"吗？`, '提示', {
    ...
    try {
      const res: any = await request.delete(`/dev-api/yjnb/videoCourse/${row.id}`)
      if (res.code === 200) {
...
const toggleRecommend = async (row: any) => {
  try {
    const apiUrl = row.recommend === 1 
      ? '/dev-api/yjnb/videoCourse/recommend'
      : '/dev-api/yjnb/videoCourse/unrecommend'
  ...
  ElMessageBox.confirm(`确定要${action}课程"${row.title}"吗？`, '提示', {
    ...
    try {
      const apiUrl = row.publishStatus === 1
        ? '/dev-api/yjnb/videoCourse/unpublish'
        : '/dev-api/yjnb/videoCourse/publish'
...
const handleVideoUpload = async (file: any) => {
  ...
  try {
    const result = await uploadFile(file.raw, '视频')
    dialogForm.videoUrl = result.url
